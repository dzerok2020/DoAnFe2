      ...
      <div class="basis-3/10 mr-4">
        <div class="flex flex-col h-full">
          ...
          <div class="basis-3/20 mb-4">
            ...
            >
              <div v-for="i in users.length" :key="i">
                <image-text
                    classes-text="text-x truncate xl:whitespace-normal"
                    :label="roles.data[i - 1].name"
                    :src="roles.data[i - 1].imgUrl"
                />
              </div>
            </div>
          </div>
        ...
        <div class="flex flex-col h-full">
          <div class="basis-9/10">
            <div class="grid grid-cols-4 grid-rows-4 gap-2 h-full w-full">
              <div class="bg-sky-400 rounded" v-for="i in users">
                <div
  ...
  methods: {
    ...
    getUsers() {
      ...
      onValue(ref(database, "play-" + this.user.room), (snapshot) => {
        ...
        snapshot.forEach((child) => {
          if (child.key !== "message" && child.key !== "system") {
    ...
    countdown: function () {
      if (this.time.totalTime >= 0) {
        ...
        const time = this.time.totalTime--;
        if (time == 5) {
          push(ref(database, `play-${this.user.room}/system`), {
